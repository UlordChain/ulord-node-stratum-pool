...
function SetupForPool(logger, poolOptions, setupFinished) {
    ...
    var processPayments = function () {
        ...
        var startTimeRedis;
        var startTimeRPC;
        var processFlag = parseInt(Math.random()*1000000);
        ...
        async.waterfall([
            ...
            function (workers, rounds, callback) {
                ...
                daemon.batchCmd(batchRPCcommand, function (error, txDetails) {
                    ...
                    txDetails.forEach(function (tx, i) {
                        ...
                        /*		console.log("txDetails: " +  JSON.stringify(txDetails[i].result));	 */
                        var round = rounds[i];

                        /*		console.log("JSON.stringify round = " + JSON.stringify(round)); */
                        // update confirmations for round
                        //		console.log("tsymentProcessor.js, tx.result.confirmations = " + JSON.stringify(tx.result));
            ...
            function (workers, rounds, addressAccount, callback) {
                ...
                var trySend = function (withholdPercent) {
                    ...
                    function sendBillsToDB(dataArray) {
                        for (var a in addressAmounts) {
                            commandArray.push(['hincrbyfloat', coin + ':bills', a.toString(), addressAmounts[a]]);

                        }
                    }

                    redisClient.multi(commandArray).exec(function (err) {
                        if (err) {
                            callback(true);
                            return;
                        }
            ...
            function payBills(payTrack) {
                daemon.cmd('sendmany', ["", payTrack], function (result) {
                    // check for failed payments, there are many reasons
                    // util.logToFile("payTracks: " + JSON.stringify(payTrack) ,"./tmpLogs/payTrack.log");
                    if (result.error && result.error.code === -6) {
                        ...
                        if (result.error.message && result.error.message.includes("insufficient funds")) {
                            ...
                            // }
                            // tx has Limit size
                            /* } else if(result.error.message && result.error.message.includes("Transaction too large")){ */
                        ...
                        if (txid != null) {
                            ...
                            var commandBills = []
                            Object.keys(payTrack).forEach(function (v, i) {
                                commandBills.push(['hdel', 'ulord:bills', v]);
                            })
                            redisClient.multi(commandBills).exec(function (err, result) {
                                if (err) {
                                    console.log('commandBills failured!');
                                    return;
                                }
                                redisClient.zadd('ulord:payedBills', Date.now(), JSON.stringify(payData), function (err, result) {
                                    if (err) {
                                        console.log('commandPayedBill failured!')
                                        return
                                    }
                                    console.log('commandPayedBill successed!')
                                })
                            })
            ...
            redisClient.hgetall('ulord:bills', function (err, result) {
                var count = 0;
                var payTrack = {};
                for (var address in result) {
                    count++;
                    payTrack[address] = roundTo(result[address],8);
                    if (count == 5) {
                        payBills(payTrack);
                        count = 0;
                        payTrack = {};
            ...
            // add sendmany to deal with bills here
    ...
    var getProperAddress = function (address) {
        ...
        if (address.length < 25 || address.length > 34) {
            util.logToFile("invalidAddress: " + JSON.stringify(address), 'validateAddress.log');
